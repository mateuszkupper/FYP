...
class Cell:
    def __init__(self, text, util):
        self.memory = MNMemory.Memory(tf.expand_dims(text, 1), util)
        ...
        self.p = tf.Variable(tf.zeros([1, util.largest_num_of_words_any_paragraph]), trainable=False)
    ...
    def connect(self, previous_layer):
        with tf.name_scope('cell'):
            ...
            self._o = tf.matmul(self.p, self.memory.c)
    ...
    def connect_encode(self, previous_layer, q):
        with tf.name_scope('cell'):
            ...
            self._o = tf.cond(tf.reduce_all(tf.equal(q, self.blank_word)), lambda: self._o,
                              lambda: tf.matmul(self.p, self.memory.c))
...
class DynamicCell:
    def __init__(self, text, util):
        self.X = tf.Variable(tf.random_normal([vocab_size, d], stddev=0.001), name="X")
        self.enc_cell1 = InitCell(text, util)
        self.enc_cell2 = Cell(text, util)
        self.enc_cell3 = Cell(text, util)
        self.dec_cell1 = InitCell(text, util)
        self.dec_cell2 = Cell(text, util)
        self.dec_cell3 = Cell(text, util)
        ...
        with tf.name_scope('decode'):
            ...
            for word in answer_words:
                with tf.name_scope('decode_iteration'):
                    ...
                    self.dec_cell1.connect(tf.squeeze(self.dec_cell3.a, 1))
                    if i==0:
                        answer_word = tf.transpose(tf.nn.relu(tf.matmul(self.X, tf.transpose(self.enc_cell3.o))))
                    answer_word = tf.cond(tf.reduce_all(tf.equal(s, answer_word)), lambda : s,
                                          lambda : tf.transpose(tf.matmul(self.X, tf.transpose(self.enc_cell3.o))))
